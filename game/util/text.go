...
func DrawWrappedText(screen *ebiten.Image, str string, x, y, maxWidth int, fontSize int, clr color.Color) {
	...
	for _, w := range words {
		...
		op := &text.DrawOptions{}
		op.GeoM.Translate(float64(x), float64(y+yOffset))
		op.ColorScale.ScaleWithColor(clr)
		text.Draw(screen, line, &f, op)
	}
}
